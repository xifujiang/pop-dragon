...
PIMAGE wheel[33],back1,a[8],lose,win,baozha[7],Spop[2];
...
void Init_Pop()///初始化泡泡
{
    ...
    for(i = 0; i<8; i++)
        a[i] = newimage();
    getimage(a[0],"img/ball/black.png");
    getimage(a[1],"img/ball/red.png");
    getimage(a[2],"img/ball/blue.png");
    getimage(a[3],"img/ball/orange.png");
    getimage(a[4],"img/ball/green.png");
    getimage(a[5],"img/ball/white.png");
    getimage(a[6],"img/ball/yellow.png");
    getimage(a[7],"img/ball/purple.png");
}
...
int AttachmentJudge(position pos[12][9],int i,int j)///判断是否附着
{
    // printf("i = %d j = %d\n",i,j);
    if(i%2==0)
    {
...
int Pop_drap(position pos[12][9])///泡泡掉落动画
{
    int i,j;
    int l[102][2] = {0},t = 0;
    for(i = 0; i<11; i++)
...
void mainloop()///游戏
{
    ...
    for ( ; is_run(); delay_fps(60) )
    {
        ...
        if(end_time-start_time>5 &&end_time-start_time<10)
        {
            int err;
            pthread_t ntid;///创建线程
        ...
        else if(end_time-start_time==10)
            time(&start_time);*/
    }
}
